...
export class ApiService {
  ...
  return this.http
      ...
      .pipe(
        ...
        tap(res =>
          localStorage.setItem(this.key, JSON.stringify(this.agencies))
        )
      );
  }

  public getMissionTypes = (): Observable<any[]> => {
  if (this.types) return of(this.types);
  return this.http
  ...
  return  this.http.get(environment.url + '/assets/launchstatus.json').pipe(
